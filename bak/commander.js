...
Command.prototype.customCommands = function(commands, customCommands, prefix, possibleDirs, customRoot) {
  if (!possibleDirs) possibleDirs = ['bin']
  if (!customRoot) customRoot = process.cwd()

  // manipulate options of commands
  for (var name in commands) {
    var opts = commands[name]
    if (typeof opts === 'string') commands[name] = opts = {desc: opts}
  }

  // Load custom commands
  for (var name in customCommands) {
    var opts = customCommands[name]
    // disable command
    if (!opts) {
    ...
    if (typeof opts === 'string') {
      customCommands[name] = opts = {desc: opts}
    }
    // check path of custom command
    var bin = opts.path
    if (!bin) {
      var subname = prefix + '-' + name
      var possibles = catesianProduct([possibleDirs, [subname + '.js', subname]])
      for (var possible of possibles) {
        var absolute = path.resolve(customRoot, ...possible)
      ...
      console.log(customRoot, ...bin.split('/'))
      bin = path.resolve(customRoot, ...bin.split('/'))
      if (!fs.existsSync(bin)) throw Error(`${path} doesn't exists`)
    }
    opts.path = bin
    if (commands[name]) opts.desc = '[overrided] ' + (opts.desc || commands[name].desc)
    else opts.desc = '[custom] ' + opts.desc
    commands[name] = customCommands[name]
  }

  // add commands to commander
  let newCommand
  for (const name in commands) {
    const opts = commands[name]
    const desc = opts.desc
    newCommand = this.command(name, desc, opts)
  }
  return newCommand
}
/**
...
Command.prototype.parse = function(argv) {
  ...
  if (this._execs[name] && typeof this._execs[name] != "function") {
    var sub = name === 'help' ? args[1] : name
    if (typeof this._execs[sub] === 'string') opts.path = this._execs[sub]
...
function catesianProduct(list) {
  return list.reverse().reduce((p,c) => c.reduce((q,w) => q.concat(p.map(v => v.concat(w))),[]), [[]]).map(v => v.reverse()) 
}
